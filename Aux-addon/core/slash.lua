...
function SlashCmdList.AUX(command)
    ...
    elseif arguments[1] == 'populate' and arguments[2] == 'wdb' then
		...
		print('- post bid [' .. status(aux_post_bid) .. ']')
		print('- tooltip value [' .. status(tooltip_settings.value) .. '|cffffff7f]|r' .. CMD_2)
		print('- tooltip daily [' .. status(tooltip_settings.daily) .. '|cffffff7f]|r' .. CMD_3)
		print('- tooltip merchant buy [' .. status(tooltip_settings.merchant_buy) .. '|cffffff7f]|r' .. CMD_4)
		print('- tooltip merchant sell [' .. status(tooltip_settings.merchant_sell) .. '|cffffff7f]|r' .. CMD_5)
		print('- tooltip disenchant value [' .. status(tooltip_settings.disenchant_value) .. '|cffffff7f]|r' .. CMD_6)
		print('- tooltip disenchant distribution [' .. status(tooltip_settings.disenchant_distribution) .. '|cffffff7f]|r' .. CMD_7)
