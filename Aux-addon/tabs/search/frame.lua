...
do
	...
	btn:SetHeight(25)
	btn:SetWidth(70)
	btn:SetText(color.label.enabled(RANGE_1))
...
do
	...
	btn:SetWidth(60)
	btn:Hide()
	btn:SetText(color.label.enabled(REAL_TIME))
	btn:SetScript('OnClick', function()
...
do
    ...
    checkbox:SetPoint('LEFT', name_input, 'RIGHT', 16, 25)
...
do
    local checkbox = gui.checkbox(frame.filter)
    checkbox:SetPoint('LEFT', max_level_input, 'RIGHT', 16, 25)
    checkbox:SetScript('OnClick', update_form)
    local label = gui.label(checkbox, gui.font_size.small)
    label:SetPoint('RIGHT', checkbox, 'LEFT', -6, 1)
...
favorite_searches_listing:SetColInfo{{name=AUTO_BUY, width=.13, align='CENTER'}, {name=FAVORITE_SEARCHES, width=.87}}

recent_searches_listing = listing.new(frame.saved.recent)
recent_searches_listing:SetColInfo{{name=RECENT_SEARCHES, width=1}}
