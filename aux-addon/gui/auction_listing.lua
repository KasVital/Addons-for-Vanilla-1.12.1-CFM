...
local TIME_LEFT_STRINGS = {
	color.red(SHORT), -- Short --byLichery
	color.orange(MEDIUM), -- Medium  --byLichery
	color.yellow(LONG), -- Long  --byLichery
	color.blue(VARY_LONG), -- Very Long  --byLichery
}
...
M.auctions_columns = {
    ...
    },
    {
        ...
        align = 'CENTER',
        fill = function(cell, record)
            cell.text:SetText(record.high_bidder or color.red (NO_BIDS)) --byLichery
        end,
        cmp = function(record_a, record_b, desc)
...
M.bids_columns = {
    ...
    },
    {
        ...
        fill = function(cell, record)
            local status
            if record.high_bidder then
                status = color.yellow(HIGH_BIDDER) --byLichery
            else
                status = color.red(OUTBID) --byLichery
            end
