    ...
    local modprint = function()
        SendChatMessage('— ПВП Ранг: ['..(UnitPVPRank'player' - 4)..'] '..'Прогресс: ['..p()..'%] '..'Текущие ОР: ['..c()..'] ОР до следующего ранга: ['..n()..'].', 'emote')
    end

    local modtip = function()
        GameTooltip:SetOwner(this, 'ANCHOR_NONE')
        GameTooltip:AddLine'|cffff6c6cРанг|r'
        GameTooltip:AddLine' '
        GameTooltip:AddLine('Эти итоги дают прогноз очков чести, необходимых для достижения нового ранга в конце недели. Рейтинги ПВП являются текучими и зависят от общего количества ОР, полученных на сервере, поэтому результаты могут отличаться.', 1, .8, 0, true)
        GameTooltip:AddLine' '
        GameTooltip:AddDoubleLine('|cffeec265Прогресс:|r', p()..'%')
        GameTooltip:AddDoubleLine('|cffeec265Текущие ОР:|r', c())
        GameTooltip:AddDoubleLine('|cffeec265ОР до следующего ранга:|r', n())
        GameTooltip:AddDoubleLine('|cffeec265ОР получишь на этой неделе:|r', g())
        GameTooltip:AddLine' '
        GameTooltip:AddLine('Нажать для вывода сообщения в чат.', .3, 1, .6)
        GameTooltip:Show()
    end
