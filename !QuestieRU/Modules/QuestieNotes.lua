		...
		for i, data in pairs(this.quests) do
			...
			if(data.icontype ~= "available") then
				...
				if not Quest then
					...
					Tooltip:AddLine("["..QuestieHashMap[data.questHash].questLevel.."] "..Quest["name"].." |cFF33FF00(Завершить)|r");
					Tooltip:AddLine("Сдать: |cFFa6a6a6"..QuestieHashMap[data.questHash].finishedBy.."|r",1,1,1);
				end
			else
				questOb = nil
				...
				Tooltip:AddLine("Получить у: |cFFa6a6a6"..QuestieHashMap[data.questHash].startedBy.."|r",1,1,1);
				if questOb ~= nil then
					Tooltip:AddLine("Описание: |cFFa6a6a6"..questOb.."|r",1,1,1,true);
				end
				Tooltip:AddLine("Шифт+Щелчок: |cFFa6a6a6Вручную завершить задание!|r",1,1,1);
			end
		end
		if(NOTES_DEBUG and IsAltKeyDown()) then
			Tooltip:AddLine("!DEBUG!", 1, 0, 0);
			Tooltip:AddLine("ИДЗадания: "..this.data.questHash, 1, 0, 0);
		end
	...
	if ( IsShiftKeyDown() and Tooltip ) then
		local finishQuest = function(quest)
			if (quest.icontype == "available") then
				...
				DEFAULT_CHAT_FRAME:AddMessage("Завершение задания |cFF00FF00\"" .. questName .. "\"|r и исходного задания: "..hash)
				Questie:Toggle()
			end
		end
