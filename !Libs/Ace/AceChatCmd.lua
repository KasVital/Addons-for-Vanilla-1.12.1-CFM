...
function AceChatCmd:result(...)
	ace:print(self.app.disabled and ACE_ADDON_STANDBY.." " or "",
		format("|cffffff78%s:|r %s", self.app.name, ace.concat(arg))
	)
end
...
function AceChatCmd:status(text, val, map)
	...
	self:result(text, " ", ACE_TEXT_NOW_SET_TO, " ", format("[|cfff5f530%s|r]", val or ACE_CMD_REPORT_NO_VAL))
end

function AceChatCmd:report(hdr, def)
	if( not def ) then def = hdr; hdr = nil end
	ace:print(hdr or format("|cffffff78%s:|r %s",
	...
	for _, ref in ipairs(def) do
		...
		ace:print((ref.indent and (strrep(" ", strlen(" - "))..strrep("   ", ref.indent)) or  " - "),
			format("%s [|cfff5f530%s|r]", ref.text, val or ACE_CMD_REPORT_NO_VAL)
		)
	end
end
...
function AceChatCmd:Register(handler)
	...
	tinsert(self.options, 1, {option = "about", desc = ACE_CMD_OPT_HELP_DESC})
	
	-- Create a closure for printing option lines, since it's done so much
	self.printOpt	= function(l,t) ace:print(format(" - |cffffff78%s:|r %s",l,t)) end
...
function AceChatCmd:ProcessCommand(msg)
	...
	if( strfind(strlower(msg), "about") == 1 ) then
...
function AceChatCmd:ProcessOptions(list, msg, handler, method, opttext)
	...
	if( i ) then
		local opt = list[i]
		if( opt.args and (args ~= "") and self:ProcessOptions(opt.args, args, opt.handler or handler, 
			opt.method or method, opttext and opttext.." "..opt.option or opt.option) ) then
		return TRUE
	elseif( opt.method and ((args ~= "") or (not opt.input)) and (strlower(args) ~= "about") ) then
		handler = opt.handler or handler
		handler[opt.method](handler, args)
		return TRUE
	elseif( opt.input and (args == "") or (opt.args and (not method)) or (strlower(args) == "about") ) then
		self:OptionHeader(opt, opttext)
		self:DisplayArgUsage(opt)
		return TRUE
		end
	elseif( ((cmd or "") ~= "") and (cmd ~= "about") and (not method) ) then
...
function AceChatCmd:DisplayAddonInfo()
	self:CommandHeader()
	if( self.commands ) then self.printOpt(ACE_TEXT_COMMANDS, ace.concat(self.commands, " | ")) end
...
function AceChatCmd:OptionHeader(option, opttext)
	local args
	if( option.args ) then
		args = self:ConcatOptions(option.args, " | ")
	end
	if( not args ) then
		args = "- "..(option.desc or ACE_CMD_USAGE_NOINFO)
	end
	ace:print(format("|cffd8c7ff%s %s|r %s", self.commands[1], opttext and opttext.." "..option.option or option.option, args))
end
...
function AceChatCmd:DisplayUsage()
	...
	ace:print(format(ACE_CMD_USAGE_HEADER, self.commands[1], self:ConcatOptions(self.options, " | ")))
...
function AceChatCmd:CommandHeader()
	ace:print(self.app.disabled and ACE_ADDON_STANDBY.." " or "",
		format("|cffffff78[%s v%s]|r : %s", self.app.name, self.app.version, self.app.description)
	)
end
