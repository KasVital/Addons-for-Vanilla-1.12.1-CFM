...
function SmartHeal:InitClickMode() ---grid
	...
	--grid 
    if (Grid) then SH_GridCustomClick=GridCustomClick end
    --if (Grid) then SH_GridFrame_OnClick=GridFrame_OnClick end
...
function SmartHeal:setClickMode(status) --grid
	...
	if (status) then
        ...
        GridCustomClick=	 function() return SmartHeal:GridCustomClick() end
       -- GridFrame_OnClick=	 function() return SmartHeal:GridFrame_OnClick() end 
	else
        ...
        --grid
        GridCustomClick=SH_GridCustomClick
     --   GridFrame_OnClick=SH_GridFrame_OnClick
...
function SmartHeal:GridCustomClick() --grid
	...
	elseif KeyDownType then
		SmartHeal:ClickHeal(KeyDownType..arg1,unit)
	elseif ( type(SH_GridCustomClick)=="function") then 
		return SH_GridCustomClick()
	end
end--]]
...
function SmartHeal:ClickHeal(button,unit)
	...
	if (UnitIsUnit("player",unit) or UnitIsUnit("target",unit)) then
		...
		-- Resume Attack setup
		if (SmartHeal.Attacking) then SmartHeal.ResumeAttack=1 end	
		
		if(not UnitExists("target")) then clearTarget=1 end
